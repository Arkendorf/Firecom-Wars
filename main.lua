...
function love.mousepressed(mX, mY, button)
  if chars[selected][3] - chars[selected][1] < 1 and chars[selected][4] - chars[selected][2] < 0.5 then
    if moveValid(round(chars[selected][1]), round(chars[selected][2]), round((mX - 32 + x) / 64) * 64, round((mY - 32 + y) / 64) * 64) == true then
...
function love.draw()
  ...
  orderE = {{scout, scout1, enemies[1][1] - x, enemies[1][2] - y - 32}}
    ...
    for tilesAcross = 1, #map[1] do
      if mapRevealed[rowsDown][tilesAcross] == 1 then
        ...
        for i = 1, #orderE do -- Enemies
          if enemies[i][3] == tilesAcross * 64 - 64 and enemies[i][4] == rowsDown * 64 - 64 then
